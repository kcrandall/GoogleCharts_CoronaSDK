...
function M.construct(options)
	...
	for i = 1, #data do 
		for d = 1, #data[i] do 
			local notString = false
			for s = 1, string.len(data[i][d]) do 
				if string.sub( data[i][d], i, i) == "{" then 
					notString = true 
					break
				end
			end
	...
	if options.version == "1.1" then
		...
		<head>
		  ...
		  <script type="text/javascript">
		    ...
		    HTML = HTML .. [[
		      ...
		      var chart = new google.charts.Bar(document.getElementById(']] .. elementByID .. [['));
		...
		<body>
		  <div id="]] .. elementByID .. [[" style="width: ]] .. width .. [[px; height: ]] .. height .. [[px"></div>
		</body>
		</html>
		]]
		...
		HTML = [[ 
		  ...
		  <head>
		    ...
		    HTML = HTML .. [[
		        ...
		        var chart = new google.visualization.BarChart(document.getElementById(']] .. elementByID .. [['));
